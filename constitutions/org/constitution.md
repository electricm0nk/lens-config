...
**Corollary — Installer Is the Authority:** The `install.sh` script in the release module (`bmad.lens.release/_bmad/lens-work/scripts/install.sh`) is the single authority for adapter content. Manually created or modified command stubs are not authoritative. The installer is idempotent and safe to re-run on every preflight.
